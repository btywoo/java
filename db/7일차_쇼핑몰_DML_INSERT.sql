...
# abc123 회원이 ACC001 제품을 장바구니에 1개 담았을 때 쿼리
...
values('CLO001', 'abc123', 2, '구매', now()), ('ACC001', 'abc123', 1, '구매', now());
# 장바구니에서 delete 하는 코드
...
# abc123 회원이 로그인 시도가 성공했을 때 실행해야 하는 쿼리
update member set me_fail = 0 where me_id = 'abc123';

# abc123 회원이 비밀 번호 찾기를 시도해서 인증 코드가 123asd가 발급됐을 때 실행해야 하는 쿼리
insert into code(co_me_id, co_code, co_limit) values('abc123', '123asd', date_add(now(), interval 5 minute));

#abc123 회원이 인증코드를 입력해서 비번을 abc1234로 수정했을 때 실행해야 하는 쿼리
